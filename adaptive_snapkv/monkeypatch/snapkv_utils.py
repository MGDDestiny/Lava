...
class LavaKVCluster():
    ...
    def store(self, key_states, query_states, value_states, attn_score, sparsity):
        ...
        self.sorted_attn_score,self.sorted_attn_score_indices = self.attn_score.sort(dim=-1,descending=True)
    ...
    def reset(self):
        ...
        del self.sorted_attn_score
        del self.sorted_attn_score_indices

    def calcul_attn_score(self, key_states, query_states, value_states):
        ...
        # attn_weights_mean_pooling: [bsz, num_heads, kv_seq_len]
        value_norms = torch.norm(value_states[...,:-self.window_size,:],p=1,dim=-1).max(dim=-1).values # [bsz, num_heads]
        attn_weights_mean_pooling = attn_weights_mean_pooling * value_norms.unsqueeze(-1)

        if self.gqa_support:
            attn_weights_mean_pooling = attn_weights_mean_pooling.view(attn_weights_mean_pooling.shape[0],num_heads//self.num_key_value_groups,self.num_key_value_groups,-1)
            attn_weights_mean_pooling = attn_weights_mean_pooling.max(dim=-2).values

        attn_weights_mean_flat = torch.flatten(attn_weights_mean_pooling)
        es_probs = attn_weights_mean_flat / (attn_weights_mean_flat.sum(dim=-1,keepdim=True) + 1e-16)
        entropy = -torch.sum(es_probs * torch.log(es_probs+ 1e-16), dim=-1)
        max_entropy = np.log(attn_weights_mean_flat.shape[-1])
        sparsity = entropy / max_entropy

        return attn_weights_mean_pooling, sparsity
    ...
    def init4flash(self):
        _device = self.key_states[0].device
        k_lens = []
        klen_sum = 0
        max_seqlen_k = 0
        self.cu_klen = 0

        for head_idx in range(self.num_heads//self.num_key_value_groups):
            l = self.key_states[head_idx].shape[0]
            k_lens.append(l)
            max_seqlen_k = max(max_seqlen_k, l)
            klen_sum += l

        self.init_metadata(self.num_heads, k_lens, klen_sum, max_seqlen_k, _device)

    def init_metadata(self, num_heads, k_lens, klen_sum, max_seqlen_k, _device):
        # init metadata
        self.head_lens = torch.tensor(k_lens, dtype=torch.int32, device=_device)
        self.klen_sum = klen_sum
        self.max_seqlen_k = max_seqlen_k
    ...
    def re_update_kv_gqa(self):
        _device = self.key_states[0].device
        bsz, num_heads, q_len, head_dim = self.bsz, self.num_heads, self.q_len, self.head_dim
        if self.base_capacity > self.mean_seq_lens:
            return  

        self.mean_seq_lens = self.base_capacity
        
        if self.layer_idx >= self.skip:
            adaptive_attn_score = self.sorted_attn_score
            length = adaptive_attn_score.size(dim=-1)
            if self.normalize:
                ratio_weight = self.sorted_attn_score[...,:self.base_capacity].sum(dim=-1,keepdim=True)/self.sorted_attn_score.sum(dim=-1,keepdim=True)
                adaptive_attn_score = adaptive_attn_score*ratio_weight
            adaptive_attn_score = adaptive_attn_score.reshape(bsz,length*num_heads//self.num_key_value_groups)
            if self.sorted_indices is None:
                ...
                sorted_indices = self.sorted_indices[:, :num_heads*self.base_capacity//self.num_key_value_groups]
            ...
            head_adaptive_capacity = torch.ones((bsz,num_heads),device=_device,dtype = self.sorted_attn_score_indices.dtype) * self.base_capacity

        head_indices = []
        max_len = max(state.shape[0] for state in self.key_states)
        
        for head_idx in range(num_heads//self.num_key_value_groups):
            cur_len = self.key_states[head_idx].shape[0]
            remove_cache = cur_len - self.window_size - head_adaptive_capacity[0][head_idx]
            if cur_len > self.window_size + head_adaptive_capacity[0][head_idx]:
                # 计算需要保留的索引
                keep_prefix = cur_len - self.window_size - remove_cache
                indices = torch.cat([
                    torch.arange(keep_prefix, device=_device),
                    torch.arange(cur_len - self.window_size, cur_len, device=_device)
                ])
                head_indices.append(indices)
            else:
                head_indices.append(None)
        
        for head_idx in range(num_heads//self.num_key_value_groups):
            if head_indices[head_idx] is not None:
                self.key_states[head_idx] = torch.index_select(
                    self.key_states[head_idx], 0, head_indices[head_idx]
                )
                self.value_states[head_idx] = torch.index_select(
                    self.value_states[head_idx], 0, head_indices[head_idx]
                )

    # update kv with gqa_support
    def update_kv_gqa(self):

        _device = self.key_states.device
        bsz, num_heads, q_len, head_dim = self.bsz, self.num_heads, self.q_len, self.head_dim
        origin_heads_key_states = torch.split(self.key_states, 1, dim=1)
        origin_heads_value_states = torch.split(self.value_states, 1, dim=1)
        if self.base_capacity > self.attn_score.size(-1):
            return
        ...
        if self.layer_idx >= self.skip:
            ...
            head_adaptive_capacity = torch.ones((bsz,num_heads),device=_device,dtype = self.sorted_attn_score_indices.dtype) * self.base_capacity
        sorted_attn_score_indices = self.sorted_attn_score_indices.split(1,dim=1)
        self.key_states = []
        self.value_states = []
        assert bsz == 1
        ...
        for head_idx in range(num_heads//self.num_key_value_groups):
            ...
            # NOTE: flatten view
            self.key_states.append(selected_k.view(-1, head_dim))
            self.value_states.append(selected_v.view(-1, head_dim))
    ...
    def update_kv_wo_gqa(self,  origin_key_states, query_states, origin_value_states):
        ...
        _device = key_states.device
        bsz, num_heads, q_len, head_dim = query_states.shape
        attn_score= self.calcul_attn_score(key_states,query_states,value_states)
        origin_heads_key_states = torch.split(key_states, 1, dim=1)
        origin_heads_value_states = torch.split(value_states, 1, dim=1)
